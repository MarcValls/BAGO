...
def check_cpp_local() -> dict:
    # Local socket probe of the BAGO cpp-local runtime. Tries 127.0.0.1:11435 by default.
    host = os.environ.get("BAGO_CPP_HOST", "127.0.0.1")
    port = int(os.environ.get("BAGO_CPP_PORT", "11435"))
    t0 = time.perf_counter()
    try:
        with socket.create_connection((host, port), timeout=2.0) as s:
            s.settimeout(2.0)
            # cpp-local echoes a banner; read 32 bytes or whatever's available
            try:
...
CHECKS = [check_ollama, check_cpp_local, check_knowledge, check_supervisor, check_release]
