...
def validate_static_truth(root: Path) -> list[str]:
    ...
    projection = _json(root / "backend/contracts/readme_projection.v1.json")
    governance = projection.get("governance", {})
    next_phase = str(governance.get("next_gateway_phase", ""))

    required = {
        "generated version": f"| Version de producto | `{version}` |",
        ...
        "boundary status": (
            "| Frontera unica de ejecucion | "
            f"`{governance.get('unique_execution_boundary', '')}` |"
        ),
        "strong-human status": (
            "| Strong Human Identity Proof | "
            f"`{governance.get('strong_human_identity_verified', '')}` |"
        ),
        "scheduler state": (
            "| Scheduler + DelegationGrant | "
            f"`{governance.get('scheduler_delegation_status', '')}` |"
        ),
        "next execution phase": next_phase,
        "generated block provenance": "Generated by backend/scripts/generate_readme_projection.py",
    }
