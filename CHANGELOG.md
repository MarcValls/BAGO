...
- **Escalacion iterativa sobre TODOS los providers activos**: Cuando un modelo falla (auth, quota, conexion), BAGO itera sobre todos los providers con credenciales validas hasta encontrar uno que funcione. Antes solo probaba uno y fallaba silenciosamente.
  - _escalate_candidates() devuelve una lista completa de candidatos ordenados por task-fit.
  - _cloud_escalation_candidates() itera sobre todos los clouds + locales como fallback.
  - _quality_cloud_retry() y ctx-overflow prueban cada candidato con mark_provider_degraded() en fallo.

- **Validacion de credenciales**: Las API keys se validan antes de ser consideradas "activas".
...
- **Copilot lee token de credentials.json**: Si GITHUB_TOKEN no esta en env, lo busca en el archivo de credenciales.

- **Codex prioriza OAuth token sobre env var invalida**: Si OPENAI_API_KEY es invalida, salta al OAuth token de ChatGPT Plus.
...
- Retrocompatible: _escalate_model() y _cloud_escalation_for_quality() siguen existiendo como wrappers que devuelven el primer candidato.
- Sin cambios en CLI ni en comandos de usuario.
