...
function createServer(bundle, checksum, options = {}) {
  return http.createServer((req, res) => {
    ...
    if (start > 0 && options.rejectRange) {
      res.writeHead(416, { 'Content-Range': `bytes */${bundle.length}` });
      res.end();
      return;
    }
...
async function main() {
  ...
  try {
    ...
    const rejectServer = createServer(bundle, checksum, { rejectRange: true });
    await new Promise(resolve => rejectServer.listen(0, '127.0.0.1', resolve));
    const rejectPort = rejectServer.address().port;
    const rejectRelease = {
      ...release,
      assets: release.assets.map(asset => asset.name === 'bundle.zip'
        ? { ...asset, browser_download_url: `http://127.0.0.1:${rejectPort}/bundle.zip` }
        : asset
      )
    };
    const manager416 = new ReleaseJobManager({
      rootDir: path.join(root, 'jobs-root-416'),
      allowedHosts: ['127.0.0.1'],
      allowInsecureHosts: ['127.0.0.1']
    });
    try {
      const cacheDir = path.join(root, 'jobs-root-416', 'cache', 'v9.9.9');
      fs.mkdirSync(cacheDir, { recursive: true });
      fs.writeFileSync(path.join(cacheDir, 'bundle.zip.part'), bundle.subarray(0, Math.floor(bundle.length / 3)));
      const created416 = manager416.startPrepare({ release: rejectRelease, target, action: 'update' });
      const ready416 = await manager416.waitFor(created416.id, ['ready'], 60000);
      assert.strictEqual(ready416.state, 'ready');
      assert.strictEqual(fs.existsSync(ready416.bundle_path), true);
      assert.strictEqual(ready416.verification.actual_sha256, sha256);
    } finally {
