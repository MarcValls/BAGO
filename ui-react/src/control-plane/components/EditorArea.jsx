  ...
  return (
    <main className="cp-editor">
      <div className="cp-editor-tabs">
        <button type="button" className={`cp-editor-tab ${activeRoom === 'chat' ? 'is-active' : ''}`} onClick={() => navigate('chat')}>
          <Icon name="chat" size={14} /> Chat
        </button>
        {activeRoom !== 'chat' && (
          <button type="button" className="cp-editor-tab is-active">
            <Icon name={(ROOMS.find((room) => room.id === activeRoom) || {}).icon || 'dashboard'} size={14} />
            {TITLES[activeRoom] || activeRoom}
            <span className="cp-editor-tab-close" onClick={(event) => { event.stopPropagation(); navigate('chat') }}>×</span>
          </button>
        )}
        {selectedFile && (
          <button type="button" className={`cp-editor-tab ${activeRoom === 'file' ? 'is-active' : ''}`} onClick={() => navigate('file')}>
            <Icon name="file" size={14} /> {selectedFile.name}
            <span className="cp-editor-tab-close" onClick={(event) => { event.stopPropagation(); onCloseFile?.() }}>×</span>
          </button>
        )}
      ...
      <div className="cp-editor-body">
        {activeRoom === 'chat' && <div className="cp-chat-panel"><ChatView control={chatControl} center={chatCenter} /></div>}
        {activeRoom === 'file' && selectedFile && (
          <div className="cp-file-viewer">
            <div className="cp-file-head"><Icon name="file" size={14} /><span>{selectedFile.path}</span></div>
            {selectedFileLoading ? <div className="cp-loading">Cargando archivo…</div> : <pre className="cp-file-content">{selectedFileContent}</pre>}
          </div>
        )}
        {activeRoom === 'dashboard' && <DashboardView context={context} onSetContext={setContext} onAction={onAction} />}
        {activeRoom === 'installations' && <InstallationsView context={context} onSetContext={setContext} onAction={onAction} onOpenTerminal={onOpenTerminal} />}
        {activeRoom === 'patchbay' && <PatchbayView context={context} onSetContext={setContext} onAction={onAction} />}
        ...
        {activeRoom === 'health' && <HealthView context={context} onSetContext={setContext} onAction={onAction} />}
