...
const _cfg = getUiConfig()

const DEFAULT_KIT = {
  installation: { id: 'local', label: 'BAGO local', version: _cfg.version, status: 'ready' },
    ...
    try {
      ...
      if (session?.provider) {
        ...
        next.installation = {
          ...
          version: session.bago_version || next.installation?.version || _cfg.version,
        }
      }
