...
export function ControlPlane() {
  ...
  const paletteActions = useMemo(() => {
    ...
    const base: PaletteItem[] = [
      ...
      // ─── Configuración (auto-generada por tests) ───
      { id: 'auto-config',     label: 'Auto-configurar BAGO (genera config con tests)', group: 'Configuración', icon: 'sparkle', action: () => void runCommand('/auto-config start') },
      { id: 'auto-config-status', label: 'Ver estado de auto-configuración',          group: 'Configuración', icon: 'live',    action: () => void runCommand('/auto-config status') },
      { id: 'auto-config-apply',  label: 'Aplicar última auto-configuración',         group: 'Configuración', icon: 'check',   action: () => void runCommand('/auto-config apply') },
      { id: 'blacklist-show',     label: 'Ver modelos bloqueados',                     group: 'Configuración', icon: 'warning', action: () => void runCommand('/blacklist show') },
