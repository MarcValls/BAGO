...
def build_context(handler: Any) -> RequestContext:
    base_timeout = float(getattr(handler, "chat_timeout_s", 120.0) or 120.0)
    # Si el modelo activo pasa por el translation middleware, el adapter
    # subyacente puede tardar más (granite3.2:8b ~90-180s por respuesta).
    # El watchdog del chat no debe cortar antes que el adapter, así que
    # subimos el timeout automáticamente. El usuario puede seguir
    # sobreescribiendo con BAGO_API_CHAT_TIMEOUT.
    mgr = getattr(handler, "session_mgr", None)
    if mgr is not None and bool(getattr(mgr, "_translation_middleware_active", False)):
        try:
            tm_cfg = mgr.config.get("translation_middleware", {}) if mgr.config else {}
            adapter_to = float(tm_cfg.get("adapter_timeout_s", 180.0) or 180.0)
            # Margen: el middleware hace 2 llamadas al traductor + 1 al
            # modelo subyacente. Sumamos 30s de margen.
            needed = adapter_to + 30.0
            if base_timeout < needed:
                base_timeout = needed
        except Exception:
            pass
