...
def handle(handler: "BaseHTTPRequestHandler", body: dict[str, Any]) -> None:
    ...
    # FIX v0.3: el handler siempre retorna 200 OK con un payload que
    # indica el resultado real. Usar 400 para "comando desconocido" o
    # "result.ok = false" confunde a la UI (que puede mostrar errores
    # 4xx como fallos de transporte) y bloquea el replay determinista
    # del shadow. El estado del comando vive en `result.ok` y
    # `result.message`; el status HTTP refleja solo el transporte.
