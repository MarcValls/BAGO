...
describe('canonical shell action registry', () => {
  it('uses the same product order for sidebar, shortcuts and palette', () => {
    expect(NAVIGATION_ORDER).toEqual(['home', 'chat', 'workspace', 'context', 'pipeline', 'evidence', 'system', 'agents', 'interpreter', 'github-auth']);
    expect(NAVIGATION_GROUPS.flatMap((group) => group.items.map((item) => item.shortcut))).toEqual(['Ctrl+1', 'Ctrl+2', 'Ctrl+3', 'Ctrl+4', 'Ctrl+5', 'Ctrl+6', 'Ctrl+7', 'Ctrl+8', 'Ctrl+9', 'Ctrl+0']);
  ...
  it('exposes object and verb labels plus the reusable workspace action', () => {
    const noop = vi.fn();
    const actions = createShellActions({ navigate: noop, openPanel: noop, openWorkspace: noop, toggleSidebar: noop, toggleFocus: noop, toggleReview: noop, runCommand: noop, runContextCommand: noop, sidebarCollapsed: false, globalMode: 'normal' });
