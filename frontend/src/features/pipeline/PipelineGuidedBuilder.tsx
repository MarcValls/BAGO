...
export function PipelineGuidedBuilder(props: Props) {
  ...
  return <section className="pipeline-builder pipeline-guided-builder" aria-label="Creador guiado de Pipeline">
    ...
    <div className="pipeline-builder-stage">
      ...
      {stage === 3 && <div className="pipeline-builder-fields"><h3>Capacidades y dependencias</h3><p>Selecciona solo las piezas necesarias. Se resolverán antes de iniciar.</p><div className="pipeline-option-grid">{CAPABILITIES.map((item) => <label key={item.id}><input type="checkbox" checked={capabilities.includes(item.id)} onChange={() => toggle(item.id, capabilities, setCapabilities)} /><span><strong>{item.label}</strong><small>{item.id}</small></span></label>)}</div><button className="text-button" type="button" onClick={props.onOpenPackages}>Revisar catálogo de paquetes</button></div>}
