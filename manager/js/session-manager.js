...
function pmRenderSessionList() {
  ...
  box.innerHTML = pmSessions.map((item) => (
    '<button type="button" class="pm-row ' + (pmSession && pmSession.session_id === item.sid ? 'selected' : '') + '" data-session-id="' + escapeHtml(item.sid) + '">' +
