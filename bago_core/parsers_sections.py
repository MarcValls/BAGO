...
def add_session_parsers(sub: argparse._SubParsersAction) -> None:
    ...
    autonomous_parser = sub.add_parser("autonomous", help="Gestiona el modo autónomo de BAGO")
    autonomous_sub = autonomous_parser.add_subparsers(dest="autonomous_cmd")
    autonomous_sub.add_parser("status", help="Muestra si la autoevolución al arrancar está activa")
    autonomous_sub.add_parser("on", help="Activa la autoevolución al arrancar")
    autonomous_sub.add_parser("off", help="Desactiva la autoevolución al arrancar")
    autonomous_sub.add_parser("start", help="Activa la autoevolución y abre BAGO")
    autonomous_sub.add_parser("total", help="Activa la autoevolución y la ejecución automática de herramientas")
    agent_parser = autonomous_sub.add_parser("agent", help="Ejecuta el agente autónomo con un objetivo")
    agent_parser.add_argument("goal", nargs="?", default="", help="Objetivo en lenguaje natural")
    autonomous_sub.add_parser("tui", help="Abre el dashboard TUI de BAGO")
    ...
    session_parser = sub.add_parser("session", help="Gestiona SessionManager sin interfaz")
    session_parser.add_argument("session_args", nargs=argparse.REMAINDER)
    ...
    install_parser.add_argument("--no-context-menu", action="store_true", help="No registra 'Abrir con BAGO' en menu contextual")
...
def add_ops_parsers(sub: argparse._SubParsersAction) -> None:
    ...
    workspace_parser = sub.add_parser("workspace", help="Registry multi-workspace para BAGO")
    workspace_parser.add_argument("--root", default="")
    workspace_sub = workspace_parser.add_subparsers(dest="workspace_cmd")
    workspace_sub.add_parser("list", help="Lista workspaces registrados")
    workspace_status = workspace_sub.add_parser("status", help="Muestra workspace activo")
    workspace_status.add_argument("--root", default=argparse.SUPPRESS, help=argparse.SUPPRESS)
    workspace_add = workspace_sub.add_parser("add", help="Registra un workspace")
    workspace_add.add_argument("--name", required=True)
    workspace_add.add_argument("--path", required=True)
    workspace_remove = workspace_sub.add_parser("remove", help="Elimina un workspace")
    workspace_remove.add_argument("--name", required=True)
    workspace_select = workspace_sub.add_parser("select", help="Selecciona workspace activo")
    workspace_select.add_argument("--name", required=True)

    knowledge_parser = sub.add_parser("knowledge", help="Federacion de conocimiento remoto")
    knowledge_parser.add_argument("--root", default="")
    knowledge_sub = knowledge_parser.add_subparsers(dest="knowledge_cmd")
    knowledge_sub.add_parser("source-list", help="Lista fuentes federadas")
    source_add = knowledge_sub.add_parser("source-add", help="Registra o actualiza fuente remota")
    source_add.add_argument("--name", required=True)
    source_add.add_argument("--url", required=True)
    source_add.add_argument("--format", default="auto", choices=("auto", "json", "jsonl", "text"))
    source_remove = knowledge_sub.add_parser("source-remove", help="Elimina fuente remota")
    source_remove.add_argument("--name", required=True)
    pull_one = knowledge_sub.add_parser("pull", help="Sincroniza una fuente remota")
    pull_one.add_argument("--name", required=True)
    pull_one.add_argument("--limit", type=int, default=100)
    pull_all = knowledge_sub.add_parser("pull-all", help="Sincroniza todas las fuentes")
    pull_all.add_argument("--limit", type=int, default=100)
    ...
    agent_spawn.add_argument("--delegates-to", default="")
    ...
    agent_allow = agent_sub.add_parser("allow", help="Permite que un agente delegue a otro")
    agent_allow.add_argument("source_agent")
    agent_allow.add_argument("target_agent")
    agent_deny = agent_sub.add_parser("deny", help="Revoca permiso de delegacion entre agentes")
    agent_deny.add_argument("source_agent")
    agent_deny.add_argument("target_agent")
    agent_permissions = agent_sub.add_parser("permissions", help="Muestra permisos de delegacion de un agente")
    agent_permissions.add_argument("agent_id")
    agent_delegate = agent_sub.add_parser("delegate", help="Registra una delegacion permitida entre agentes")
    agent_delegate.add_argument("source_agent")
    agent_delegate.add_argument("target_agent")
    agent_sub.add_parser("status", help="Muestra consonancia entre agentes")

    release_job_parser = sub.add_parser("release-job", help="Gestiona release jobs sin abrir Electron")
    release_job_parser.add_argument("release_args", nargs=argparse.REMAINDER)

    chain_parser = sub.add_parser("chain", help="Gestiona cadenas del Patch Manager sin interfaz")
    chain_parser.add_argument("--json", action="store_true", help="Salida JSON")
    chain_sub = chain_parser.add_subparsers(dest="chain_cmd")
    def _chain_json(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
        p.add_argument("--json", action="store_true", default=argparse.SUPPRESS, help=argparse.SUPPRESS)
        return p
    _chain_json(chain_sub.add_parser("list", help="Lista cadenas"))
    chain_show = chain_sub.add_parser("show", help="Muestra una cadena")
    chain_show.add_argument("chain_id")
    _chain_json(chain_show)
    chain_new = chain_sub.add_parser("new", help="Crea una cadena")
    chain_new.add_argument("chain_id")
    chain_new.add_argument("--name", default="")
    chain_new.add_argument("--mode", default="serial", choices=("serial", "parallel", "mixed", "atomic"))
    _chain_json(chain_new)
    chain_delete = chain_sub.add_parser("delete", help="Elimina una cadena")
    chain_delete.add_argument("chain_id")
    _chain_json(chain_delete)
    chain_export = chain_sub.add_parser("export", help="Exporta el registry de cadenas")
    chain_export.add_argument("--output", default="")
    _chain_json(chain_export)
    chain_import = chain_sub.add_parser("import", help="Importa un registry de cadenas")
    chain_import.add_argument("file")
    _chain_json(chain_import)
    _chain_json(chain_sub.add_parser("validate", help="Valida el registry de cadenas"))
    chain_stage = chain_sub.add_parser("add-stage", help="Anade una etapa a una cadena")
    chain_stage.add_argument("chain_id")
    chain_stage.add_argument("--mode", default="serial", choices=("serial", "parallel"))
    _chain_json(chain_stage)
    chain_step = chain_sub.add_parser("add-step", help="Anade un paso a una etapa")
    chain_step.add_argument("chain_id")
    chain_step.add_argument("--stage", type=int, default=0)
    chain_step.add_argument("--type", default="tool")
    chain_step.add_argument("--label", default="Nuevo nodo")
    chain_step.add_argument("--value", default="")
    chain_step.add_argument("--ref", default="")
    chain_step.add_argument("--disabled", action="store_true")
    _chain_json(chain_step)

    manager_parser = sub.add_parser("manager", help="Operaciones del gestor sin interfaz")
    manager_parser.add_argument("--json", action="store_true", help="Salida JSON")
    manager_sub = manager_parser.add_subparsers(dest="manager_cmd")
    manager_health = manager_sub.add_parser("health", help="Preflight del gestor")
    manager_health.add_argument("--json", action="store_true", default=argparse.SUPPRESS, help=argparse.SUPPRESS)
    manager_web = manager_sub.add_parser("web-chat-status", help="Comprueba el servidor local de chat web")
    manager_web.add_argument("--port", type=int, default=8080)
    manager_web.add_argument("--scan", action="store_true", help="Escanea 12 puertos desde --port")
    manager_web.add_argument("--json", action="store_true", default=argparse.SUPPRESS, help=argparse.SUPPRESS)
