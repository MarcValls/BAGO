...
| Artefactos | `dist\BAGO-Installation-Manager-4.6.1-win-x64.exe`, `release\v4\bago-v4.6.1.zip` |
...
|---|---|
| BAGO app | sesión, providers, chat, memoria, node control, workspace, knowledge |
| Gestor de instalación | instalar, actualizar, desinstalar, rollback, seleccionar copia activa, abrir con BAGO |

La vista de cadenas del Patch Manager admite modo `serial`, `parallel`, `mixed` y `atomic`. En `atomic`, cada etapa se mantiene como una unidad de un solo nodo.
...
El instalador registra por defecto **"Abrir con BAGO"** en el menú contextual de carpetas de Windows. Esa acción abre la copia activa de BAGO con la carpeta seleccionada como ruta de trabajo (`--base-path`). Si no se desea, usar `-NoContextMenu`.
...
BAGO arranca con autoevolución activa por defecto. Puedes consultar o cambiar ese estado con:

```bash
bago autonomous status
bago autonomous on
bago autonomous off
bago autonomous start
bago autonomous total
```

`bago autonomous start` activa la autoevolución y abre el REPL.
`bago autonomous total` añade auto-aprobación de herramientas para que `/autopilot` pueda completar tareas sin pedir permiso.
...
```
 ...
 | |_) / ___ \ ___) | |_| |
 |____/_/   \_\____/ \___/
           v4.6.1 — Session-First AI Chat

Bienvenido a BAGO 4.6.1. Escribe /help para ver comandos.
El contexto de sesión sobrevive al cambio de provider.
...
> **Nota:** Si el modelo por defecto no está disponible en Ollama local, BAGO 4.6.1 **auto-ajusta** al primer modelo disponible automáticamente y te avisa.

### Nuevo gestor de instalaciones

BAGO 4.6.1 separa claramente los perfiles de instalación:
...
### Registro multi-workspace (nuevo)

Para cubrir el hueco de `workspace_registry`, BAGO 4.6.1 incorpora un registry local en `.bago/state/workspace_registry.json`.

```bash
bago workspace add --name main --path C:\proyectos\BAGO
bago workspace list
bago workspace select --name main
bago workspace status
```

Esto permite gestionar varios repositorios/workspaces con un contexto activo explícito.

### Federación remota de conocimiento (nuevo)

Para cubrir el hueco de `knowledge_federation`, BAGO 4.6.1 añade fuentes remotas con ingesta deduplicada en `knowledge.db`.

```bash
bago knowledge source-add --name docs --url https://example/api/knowledge --format json
bago knowledge source-list
bago knowledge pull --name docs --limit 100
bago knowledge pull-all --limit 100
```

Orden recomendado de adopción:

1. Activar `workspace` (menos complejidad y sin dependencia de red).
2. Activar `knowledge` federation (más complejidad: red, formatos y deduplicación).
...
Si el modelo por defecto no está disponible, o siempre que arranques el REPL, BAGO 4.6.1 te ofrece elegir provider y modelo de forma interactiva:
...
BAGO 4.6.1 incluye un **motor de intenciones** que aprende automáticamente de tu estilo de conversación para decidir cuándo usar herramientas y cuándo no.
...
BAGO 4.6.1 expone una API HTTP para integraciones externas:
...
BAGO 4.6.1 puede materializar un bundle de evidencia contractual para demostrar ayuda directa e indirecta al usuario.
...
BAGO 4.6.1 permite cambiar de modelo **sin perder la sesión**. El sistema evalúa la equivalencia entre modelos y aplica la estrategia de transferencia adecuada.
...
Cuando cambias a un modelo menor, BAGO 4.6.1 **no pierde el contexto**: lo comprime por capas jerárquicamente:
...
BAGO 4.6.1 gestiona su configuración en `.bago/config.json` y las credenciales en `.bago/credentials.json`. Ya no dependes exclusivamente de variables de entorno.
...
BAGO 4.6.1 incluye un motor de RL ligero que aprende de cada interacción. En el producto estable se mantiene como `shadow/off` por defecto y no tiene autoridad de ejecución:
...
BAGO 4.6.1 puede generar planes de tareas y preparar ejecución asistida. Esta superficie es experimental y no forma parte del MVP estable hasta que tenga pruebas por escenario, permisos explícitos y evidencia.
...
BAGO 4.6.1 puede ejecutar herramientas automáticamente o pedirte confirmación antes de hacerlo, al estilo **Copilot Allow All**.
...
BAGO 4.6.1 incluye una **base de conocimiento persistente** que sobrevive a las sesiones. Almacena recuerdos, hechos y notas importantes extraídos de las conversaciones.
...
**Solución:** BAGO 4.6.1 auto-ajusta al primer modelo disponible. Si aun así falla, descarga un modelo:
...
> **Tip:** Al iniciar el REPL, BAGO 4.6.1 te ofrece una selección interactiva de providers y modelos disponibles.
...
**BAGO 4.6.1** — Session-First AI Chat
