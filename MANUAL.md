...
```
 ...
 | |_) / ___ \ ___) | |_| |
 |____/_/   \_\____/ \___/
           v4.3.0 — Session-First AI Chat

Bienvenido a BAGO 4.3.0. Escribe /help para ver comandos.
El contexto de sesión sobrevive al cambio de provider.
...
> **Nota:** Si el modelo por defecto no está disponible en Ollama local, BAGO 4.3.0 **auto-ajusta** al primer modelo disponible automáticamente y te avisa.

### Nuevo gestor de instalaciones

BAGO 4.3.0 separa claramente los perfiles de instalación:
...
Para cubrir el hueco de `workspace_registry`, BAGO 4.3.0 incorpora un registry local en `.bago/state/workspace_registry.json`.
...
Para cubrir el hueco de `knowledge_federation`, BAGO 4.3.0 añade fuentes remotas con ingesta deduplicada en `knowledge.db`.
...
Si el modelo por defecto no está disponible, o siempre que arranques el REPL, BAGO 4.3.0 te ofrece elegir provider y modelo de forma interactiva:
...
BAGO 4.3.0 incluye un **motor de intenciones** que aprende automáticamente de tu estilo de conversación para decidir cuándo usar herramientas y cuándo no.
...
BAGO 4.3.0 expone una API HTTP para integraciones externas:
...
BAGO 4.3.0 puede materializar un bundle de evidencia contractual para demostrar ayuda directa e indirecta al usuario.
...
BAGO 4.3.0 permite cambiar de modelo **sin perder la sesión**. El sistema evalúa la equivalencia entre modelos y aplica la estrategia de transferencia adecuada.
...
Cuando cambias a un modelo menor, BAGO 4.3.0 **no pierde el contexto**: lo comprime por capas jerárquicamente:
...
BAGO 4.3.0 gestiona su configuración en `.bago/config.json` y las credenciales en `.bago/credentials.json`. Ya no dependes exclusivamente de variables de entorno.
...
BAGO 4.3.0 incluye un motor de RL ligero que aprende de cada interacción. En el producto estable se mantiene como `shadow/off` por defecto y no tiene autoridad de ejecución:
...
BAGO 4.3.0 puede generar planes de tareas y preparar ejecución asistida. Esta superficie es experimental y no forma parte del MVP estable hasta que tenga pruebas por escenario, permisos explícitos y evidencia.
...
BAGO 4.3.0 puede ejecutar herramientas automáticamente o pedirte confirmación antes de hacerlo, al estilo **Copilot Allow All**.
...
BAGO 4.3.0 incluye una **base de conocimiento persistente** que sobrevive a las sesiones. Almacena recuerdos, hechos y notas importantes extraídos de las conversaciones.
...
**Solución:** BAGO 4.3.0 auto-ajusta al primer modelo disponible. Si aun así falla, descarga un modelo:
...
> **Tip:** Al iniciar el REPL, BAGO 4.3.0 te ofrece una selección interactiva de providers y modelos disponibles.
...
**BAGO 4.3.0** — Session-First AI Chat
