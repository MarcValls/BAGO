...
sys.path.insert(0, str(_THIS_DIR.parent / "core"))
try:
...
try:
    ...
    _VERSIONS_PATH = _THIS_DIR.parent.parent / "versions.json"
    BAGO_VERSION = _json.loads(_VERSIONS_PATH.read_text(encoding="utf-8"))["current"]
...
<footer style="text-align:center;padding:16px;color:var(--text-dim);font-size:0.8em;">
  BAGO v{BAGO_VERSION} — Process Monitor — auto-refresh cada {refresh}s
</footer>
</body>
</html>"""
...
def _run_tests(root: Path) -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ok("html_has_bago_version", f"BAGO v{BAGO_VERSION}" in html)
