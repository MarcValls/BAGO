...
def _animate(fast: bool = False) -> None:
    ...
    total_h   = LOGO_H + 3 + len(BOOT_MSGS) + 3
    ...
    # ── 1. REVEAL: logo aparece línea a línea ────────────────────────────────
    for i, line in enumerate(LOGO):
        rendered = _logo_line(line, i, intensity=1.0)
        if USE_COLOR:
            scan = f"\033[1;96m{'─' * LOGO_W}\033[0m"
            out(_goto(start_row + i, logo_col) + scan)
            sys.stdout.flush()
    ...
    # ── 2. ABEJA VOLANDO de izquierda a derecha ───────────────────────────────
    if not fast:
        # La abeja entra por la izquierda y vuela hasta quedar a la izquierda del logo
    ...
    # Limpiar la fila de vuelo de la abeja (ya está en el tagline)
    if not fast:
