...
def _scanner_specs(directory: str, sarif_paths: list[str] | None) -> list[dict[str, Any]]:
    specs = [{**spec, "args": [directory if arg == "{directory}" else arg for arg in spec["args"]]} for spec in SCANNER_DEFS]
...
def _run_scanner(spec: dict[str, Any], cwd: str, ci: bool) -> tuple[dict[str, Any], int, bool]:
    ...
    section = {
        ...
        "elapsed_s": elapsed_s,
        "parse_status": parse_status,
        "by_severity": _severity_counts(details[: int(spec["detail_limit"])]),
    }
...
def _resolve_changed_files(scope_root: Path, base_ref: str) -> list[str]:
    ...
    if rc != 0:
        return []
    files = [_normalize_scope_path(line.strip(), scope_root) for line in out.splitlines()]
...
def _make_verdict(score: int, min_score: int, blocker_count: int, scanner_errors: dict[str, str], total_findings: int) -> Verdict:
    if scanner_errors:
        label, verdict_id, mergeable, reason = "❌ NO MERGE (scanner error)", "no-merge", False, "scanner error"
    elif blocker_count > 0:
        label, verdict_id, mergeable, reason = "❌ NO MERGE", "no-merge", False, "blocker findings"
    elif score < min_score:
        label, verdict_id, mergeable, reason = "❌ NO MERGE", "fail", False, "score below threshold"
    elif total_findings > 0:
        label, verdict_id, mergeable, reason = "⚠️ REVIEW REQUIRED", "review-required", False, f"{total_findings} finding(s)"
    else:
        label, verdict_id, mergeable, reason = "✅ MERGE OK", "mergeable", True, "clean"
    return Verdict(
        {
            "id": verdict_id,
            "label": label,
...
def run_reviews(
    ...
    *,
    ...
    effective_base = base_ref or ("origin/main" if changed_only and ci else "HEAD" if changed_only else "")
    scope_files = _resolve_changed_files(scope_root, effective_base) if changed_only else None
    ...
    for spec in _scanner_specs(str(scope_root), sarif_paths):
        ...
        section, weighted, blocked = _apply_scope_filter(section, spec, scope_root, set(scope_files) if scope_files is not None else None)
    ...
    scanner_errors = {key: section["error"] for key, section in sections.items() if section.get("scanner_status") == STATUS_ERROR}
    ...
    return {
        ...
        "verdict_label": _make_verdict(score, min_score, blocker_count, scanner_errors, total_findings)["label"],
    }
