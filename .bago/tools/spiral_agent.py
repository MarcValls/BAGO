...
def _cmd_spawn(args: list[str]) -> int:
    ...
    while i < len(args):
        ...
        elif args[i] == '--delegates-to' and i + 1 < len(args):
            delegates_to = [item.strip() for item in args[i + 1].split(',') if item.strip()]; i += 2
        else:
            i += 1
    _save_agents_registry({agent_id: {'phase': phase % 12, 'skills': skills, 'category': 'custom', 'description': 'spawned via CLI', 'active': True, 'delegates_to': delegates_to}})
...
def _cmd_list(_args: list[str]) -> int:
    ...
    for agent in agents:
        ...
        delegates = ','.join(agent.get('delegates_to', [])) if agent.get('delegates_to') else 'none'
        print(f"{agent['id']} phase={agent['phase']} active={agent['active']} validate={agent['last_validate']} cycles={agent['cycles']} skills={skills} delegates={delegates}")
    return 0
...
def _agent_entry(registry: dict, agent_id: str) -> dict | None:
    entry = registry.get(agent_id)
    return entry if isinstance(entry, dict) and entry.get('active', True) else None


def _can_delegate(source_entry: dict, target_id: str) -> bool:
    targets = [str(item) for item in source_entry.get('delegates_to', [])]
    return '*' in targets or target_id in targets


def _cmd_allow(args: list[str]) -> int:
    if len(args) < 2:
        print('agent allow requires source and target', file=sys.stderr)
        return 1
    source, target = args[0], args[1]
    registry = load_agents_registry()
    source_entry = _agent_entry(registry, source)
    target_entry = _agent_entry(registry, target)
    if source_entry is None:
        print(f'agent not found: {source}', file=sys.stderr)
        return 1
    if target_entry is None:
        print(f'agent not found: {target}', file=sys.stderr)
        return 1
    delegates = [str(item) for item in source_entry.get('delegates_to', [])]
    if target not in delegates:
        delegates.append(target)
    source_entry['delegates_to'] = delegates
    _save_agents_registry({source: source_entry})
    print(f'allowed {source} -> {target}')
    return 0


def _cmd_deny(args: list[str]) -> int:
    if len(args) < 2:
        print('agent deny requires source and target', file=sys.stderr)
        return 1
    source, target = args[0], args[1]
    registry = load_agents_registry()
    source_entry = _agent_entry(registry, source)
    if source_entry is None:
        print(f'agent not found: {source}', file=sys.stderr)
        return 1
    source_entry['delegates_to'] = [item for item in source_entry.get('delegates_to', []) if item != target]
    _save_agents_registry({source: source_entry})
    print(f'denied {source} -> {target}')
    return 0


def _cmd_permissions(args: list[str]) -> int:
    if not args:
        print('agent permissions requires an id', file=sys.stderr)
        return 1
    agent_id = args[0]
    registry = load_agents_registry()
    entry = _agent_entry(registry, agent_id)
    if entry is None:
        print(f'agent not found: {agent_id}', file=sys.stderr)
        return 1
    targets = entry.get('delegates_to', [])
    print(f'{agent_id} delegates_to={",".join(targets) if targets else "none"}')
    return 0


def _cmd_delegate(args: list[str]) -> int:
    if len(args) < 2:
        print('agent delegate requires source and target', file=sys.stderr)
        return 1
    source, target = args[0], args[1]
    registry = load_agents_registry()
    source_entry = _agent_entry(registry, source)
    ...
    if target_entry is None:
        print(f'agent not found: {target}', file=sys.stderr)
        return 1
    if not _can_delegate(source_entry, target):
        print(f'delegation denied: {source} -> {target}', file=sys.stderr)
        return 1
    log_file = STATE_DIR / 'agent_delegations.json'
    log = load_json(log_file, [])
    if not isinstance(log, list):
        log = []
    log.append({'source': source, 'target': target, 'timestamp': timestamp_iso()})
    save_json(log_file, log[-200:])
    print(f'delegated {source} -> {target}')
    return 0
