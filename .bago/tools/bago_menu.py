...
def main() -> None:
    ...
    while True:
        ...
        if choice == "asistente" and _chat_mod:
            ...
            # Normal exit from chat -> back to M/A choice, not exit whole app
            continue

        # Modo creación
        if choice == "create":
            tools = Path(__file__).parent
            create_script = tools / "creation_mode.py"
            if create_script.exists():
                subprocess.run([sys.executable, str(create_script)], cwd=str(ROOT))
            # After create mode, re-show M/A choice
            continue

        # Modo focus
        if choice == "focus":
            tools = Path(__file__).parent
            focus_script = tools / "focus_mode.py"
            if focus_script.exists():
                subprocess.run([sys.executable, str(focus_script)], cwd=str(ROOT))
            continue

        # Modo portable
        if choice == "portable":
            tools = Path(__file__).parent
            portable_script = tools / "bago_portable.py"
            if portable_script.exists():
                subprocess.run([sys.executable, str(portable_script)], cwd=str(ROOT))
            continue
