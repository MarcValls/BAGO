...
try:
    ...
    BAGO_VERSION = _json.loads((Path(__file__).resolve().parents[2] / "versions.json").read_text(encoding="utf-8"))["current"]
...
def close_brief(brief_id: str, force: bool = False) -> TaskBrief:
    ...
    if review.get("result") not in ("approved",) and not force:
        raise ValueError(
            f"Cierre bloqueado: el brief {brief_id!r} tiene resultado '{review.get('result')}'."
        )
    ...
    try:
        ...
        try:
            ...
            results.append((
                ...
                "--force" not in str(exc)
                and ("Cierre bloqueado" in str(exc) or "no tiene revisión" in str(exc)),
                "close sin review lanza ValueError limpio",
            ))
