...
class BagoREPL:
    ...
    def _welcome_menu(self) -> bool:
        """Muestra un menú de bienvenida al arrancar BAGO. Retorna False si el usuario quiere salir."""
        if not (hasattr(sys.stdout, "isatty") and sys.stdout.isatty()):
            return True
        # Solo mostramos el menú la primera vez por sesión de proceso
        if getattr(self, "_welcome_done", False):
            return True
        ...
        choices = [
            ...
            "Empezar algo nuevo",
            "Configurar BAGO (provider/modelo)",
            "Ver estado",
            "Salir",
        ]
        idx = self._navigate(
            f"Bienvenido a BAGO {BAGO_VERSION} · ¿Qué quieres hacer hoy?",
            choices,
            hint="↑↓ navegar   Enter seleccionar   Esc/q cancelar",
        )
        if idx is None:
    ...
    def _show_command_palette(self) -> bool:
        ...
        for it in catalog:
            ...
            labels.append(f"[{idx + 1}] {it['command']}{args}  —  {it['description']}")
            if idx < 9:
                shortcuts[str(idx + 1)] = idx
        idx = self._navigate(
            "Comandos de BAGO  ·  escribe / para abrir",
            labels,
            shortcuts=shortcuts,
        )
        if idx is None:
    ...
    def _show_menu(self) -> bool:
        ...
        while True:
            labels = []
            shortcuts: dict[str, int] = {}
            for i, section in enumerate(MENU_SECTIONS):
                labels.append(f"[{i + 1}] {section['title']}  —  {section['description']}")
                if i < 9:
                    shortcuts[str(i + 1)] = i
            idx = self._navigate("Menu de funciones", labels, shortcuts=shortcuts)
            if idx is None:
    ...
    def _show_menu_section(self, section: dict[str, Any]) -> bool | None:
        ...
        for i, item in enumerate(section["items"]):
            ...
            labels.append(f"[{i + 1}] {item['command']}{args}  —  {item['description']}")
            if i < 9:
                shortcuts[str(i + 1)] = i
        idx = self._navigate(section["title"], labels, shortcuts=shortcuts)
        if idx is None:
            return None
        return self._run_menu_item(section["items"][idx])

    def _switch_confirmation_wizard(self, result: Any) -> bool:
        """Confirma con teclado un switch bloqueado por equivalencia."""
        if not self._wizard_tty_ok("/switch <provider> [modelo] --force"):
            return True
        # Soportar tanto dict (session_manager) como objeto (switch_engine)
        if isinstance(result, dict):
            model = result.get("new_model") or "?"
            provider = result.get("new_provider") or "?"
            old = f"{result.get('old_provider', '?')}/{result.get('old_model', '?')}"
        else:
            model = getattr(result, "new_model", "") or "?"
            provider = getattr(result, "new_provider", "") or "?"
            old = f"{getattr(result, 'old_provider', '?')}/{getattr(result, 'old_model', '?')}"
        labels = [
            f"Forzar cambio a {provider}/{model}",
            "Elegir otro modelo",
            "Cancelar",
        ]
        idx = self._navigate(
            f"Switch bloqueado · {old} → {provider}/{model}",
            labels,
        )
        if idx is None or idx == 2:
            print(R.dim("Switch cancelado."))
            return True
        if idx == 1:
            return self._switch_wizard()
        return self._handle_command(f"/switch {provider} {model} --force")
    ...
    def _handle_command(self, line: str) -> bool:
        ...
        if line.startswith("/switch") and not result.get("ok"):
            switch_result = result.get("result")
            # Detectar switch bloqueado por equivalencia (dict de session_manager)
            if isinstance(switch_result, dict) and switch_result.get("verdict") == "NOT_RECOMMENDED":
                return self._switch_confirmation_wizard(switch_result)
            # Detectar switch bloqueado por equivalencia (objeto SwitchResult)
            verdict = getattr(switch_result, "verdict", None)
            if verdict is not None and getattr(verdict, "name", None) == "NOT_RECOMMENDED":
                return self._switch_confirmation_wizard(switch_result)
            if getattr(switch_result, "requires_confirmation", False):
                return self._switch_confirmation_wizard(switch_result)
        ...
        try:
            ...
            while self.running:
                ...
                if _nl_cmd is None:
                    ...
                    elif stripped.upper() in {"A", "B", "G", "O"}:
                        _nl_cmd = f"/mode {stripped.upper()}"
