...
class CopilotAdapter(ProviderAdapter):
    ...
    def _resolve_wire_name(self, model_id: str) -> str:
        """Devuelve el nombre real del modelo para la API/CLI (wire_name)."""
        for info in self.list_models():
            if info.model_id == model_id:
                return info.wire_name or info.model_id
        return model_id
    ...
    def _chat_cli(self, messages: list[dict], model: str, system: str) -> ProviderResponse:
        ...
        wire_model = self._resolve_wire_name(model)
        if wire_model:
            command += ["--model", wire_model]
    ...
    def chat(
        ...
        *,
        ...
        return ProviderResponse(
            ...
            model_used=result.get("model", wire_model),
            provider=self.provider_name,
