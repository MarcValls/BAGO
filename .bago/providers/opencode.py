...
opencode.py â€” BAGO 4.1.5 OpenCode Provider Adapter

OpenCode es un agente de cÃ³digo que se conecta a mÃºltiples LLMs.
Su API es OpenAI-compatible vÃ­a proxy.
Requiere OPENCODE_API_KEY o usa la config local de OpenCode.
"""
...
OPENCODE_API = "https://api.opencode.ai/v1"


class OpenCodeAdapter(ProviderAdapter):
    """Adapter para OpenCode (OpenAI-compatible proxy)."""

    def __init__(self, config: dict | None = None):
        super().__init__("opencode", config)
        self.api_key = (config or {}).get("api_key") or os.environ.get("OPENCODE_API_KEY")
        self.base_url = (config or {}).get("base_url", OPENCODE_API).rstrip("/")
    ...
    def chat(
        ...
        *,
        ...
        try:
            ...
            return ProviderResponse(content=f"Error OpenCode: {exc}", provider=self.provider_name, model_used=model)
    ...
    def chat_stream(
        ...
        *,
        ...
        """Streaming real para OpenCode (SSE, OpenAI-compatible proxy)."""
        payload: dict[str, Any] = {
            "model": model,
            "messages": messages,
            "temperature": temperature,
    ...
    def health_check(self, timeout: float = 5.0) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus(ok=True, provider=self.provider_name, detail=f"OpenCode OK ({count} models)", models_available=count)
        except Exception as exc:
...
def _run_tests() -> int:
    ...
    print("opencode.py --test: ALL PASS")
    return 0
