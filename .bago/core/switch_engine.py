...
class SwitchEngine:
    ...
    def prepare_switch(
        ...
        if verdict == TransferVerdict.NOT_RECOMMENDED and not force:
            return SwitchResult(
                ok=False,
                message="Switch bloqueado hasta confirmar el cambio con teclado.",
                old_provider=current_provider, old_model=current_model,
...
def _run_tests() -> int:
    ...
    class RejectingEquiv:
        def transfer_verdict(self, current_model: str, new_model: str) -> TransferVerdict:
            return TransferVerdict.NOT_RECOMMENDED

    engine.equiv = RejectingEquiv()  # type: ignore[assignment]
    res = engine.prepare_switch("mock", "llama32", "mock", "llama32-1b", force=False)
    assert not res.ok
