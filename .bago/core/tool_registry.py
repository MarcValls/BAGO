...
def _get_browser():
    ...
    spec = importlib.util.spec_from_file_location(
        "browser_controller",
        Path(__file__).resolve().parents[2] / "bago_core" / "browser_controller.py",
    )
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.get_controller(headless=True)


def _tool_browser_open(url: str) -> str:
    return _get_browser().open(url)


def _tool_browser_snapshot() -> str:
    return _get_browser().snapshot()


def _tool_browser_click(ref: str) -> str:
    return _get_browser().click(ref)


def _tool_browser_fill(ref: str, text: str) -> str:
    return _get_browser().fill(ref, text)


def _tool_browser_screenshot(output_path: str = "") -> str:
    return _get_browser().screenshot(output_path=output_path or None)


BROWSER_TOOLS: list[Tool] = [
    Tool(
        name="browser_open",
        description="Abre una URL en el navegador persistente controlado por Playwright.",
        parameters={
            "type": "object",
            "properties": {
        ...
        function=_tool_browser_open,
    ),
    Tool(
        name="browser_snapshot",
        description="Captura un snapshot del DOM con refs interactivos para click/fill.",
        parameters={"type": "object", "properties": {}},
        function=_tool_browser_snapshot,
    ),
    Tool(
        name="browser_click",
        description="Hace click en un elemento del snapshot por su ref (ej. e3).",
        parameters={
            "type": "object",
            "properties": {
                "ref": {"type": "string", "description": "Referencia del elemento, ej. e3"},
            },
            "required": ["ref"],
        },
        function=_tool_browser_click,
    ),
    Tool(
        name="browser_fill",
        ...
        function=_tool_browser_fill,
    ),
    Tool(
        name="browser_screenshot",
        description="Captura pantalla del navegador persistente.",
        parameters={
            "type": "object",
            "properties": {
        ...
        function=_tool_browser_screenshot,
    ),
]

# Unir todo
BUILTIN_TOOLS = BUILTIN_TOOLS + BROWSER_TOOLS
