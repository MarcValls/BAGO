...
|---|---|
| **BAGO app** | runtime, sessions, providers, chat, node control, knowledge/workspace commands |
| **Installation manager** | install/update/rollback, active copy selection, release jobs, context menu, health checks |
...
| `bago autonomous start` | enables auto-evolution and opens the REPL |
| `bago autonomous total` | enables auto-evolution plus automatic tool execution |
| `bago chain new --mode atomic` | creates an atomic chain with one node per stage |
| `bago workspace add --name dev --path D:\work\repo` | registers and selects portable workspaces |
| `bago workspace select --name dev` | switches the active workspace context |
| `bago knowledge source-add --name docs --url https://example/api --format json` | configures a remote federation source |
| `bago knowledge pull --name docs --limit 100` | ingests remote knowledge into local `knowledge.db` with dedupe |
...
1. harden install/release reliability before new features,
2. keep the MVP frozen and evidence-backed,
3. finish Node Control/Patch Manager as backend authority first,
4. make startup and provider/session state predictable,
5. keep React UI as an optional surface, never the authority,
6. keep RL/autopilot/autoevolve shadow/off or explicitly authorized,
7. keep README, roadmap, claims, and contracts aligned with proof.

See [`docs/MVP.md`](docs/MVP.md), [`docs/MODULES.md`](docs/MODULES.md), and [`RELEASE_CHECKLIST.md`](RELEASE_CHECKLIST.md).

## Gap Coverage Order (workspace + federation)

Implementation order used for v4.x compatibility hardening:

1. `workspace` registry first (lower complexity, local-only state, no network dependency).
2. `knowledge` federation second (higher complexity: remote payload parsing, ingest dedupe, fault handling).

This order minimizes operational risk while restoring multi-workspace continuity before remote synchronization.
